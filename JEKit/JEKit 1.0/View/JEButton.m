...
- (UIImageView *)coverView {
    if(_coverView == nil) {
        ...
        if (image == nil && ([self imageForState:(UIControlStateNormal)] == nil)) {image = [UIImage je_ColoreImage:[UIColor whiteColor]]; }
        _.image = image;
        _.bor = self.bor;
...
- (void)startAnimatingCover:(BOOL)cover{
    ...
    if (_disableInLoading) {
        [self setTitleColor:[_normalTitleColor  je_Abe:1 Alpha:(1 - 0.618)] forState:UIControlStateNormal];
    }
