...
- (UILabel *)La_title{
    if (_La_title == nil) {
        _La_title = JELa(JR(0, self.contentView.height - 225, self.contentView.width, 30),nil,fontM(25),nil,(1),self.contentView);
    }return _La_title;
}

- (UILabel *)La_desc{
    if (_La_desc == nil) {
        _La_desc = JELa(JR(12, _La_title.bottom + 15, self.contentView.width - 12*2, 50),nil,font(16),kColorWhite,(1),self.contentView);;
    }return _La_desc;
}
...
- (UIButton *)Btn_finish{
    if (_Btn_finish == nil) {
        ...
        _Btn_finish = JEBtn(JR((self.width - width)/2, _Page.y - height - offset, width, height),@"立即体验".loc,fontM(16),(_Arr_colors ? [UIColor whiteColor] : _tintColor),self,@selector(dismiss),_Arr_colors,height/2,self);
        if (_Arr_colors == nil) {
            [_Btn_finish je_addImgByRadius:height/2 color:_tintColor lineWidth:1];
        }
        _Btn_finish.alpha = 0;
    }
    return _Btn_finish;
