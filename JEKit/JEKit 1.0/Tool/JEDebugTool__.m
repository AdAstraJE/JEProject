...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier{
    ...
    _La_index = JELa(JR(jkLabelMargin, 8, ScreenWidth - jkLabelMargin*2, 20),nil,font1,kHexColor(0xDC3023),(1),self.contentView).adjust();
    _La_API = JELa(JR(_La_index.x, _La_index.bottom + jkLabelMargin, _La_index.width, 0),nil,font2,kHexColor(0xE29C45),(0),self.contentView);
    _La_param = JELa(JR(_La_index.x, _La_API.bottom, _La_API.width, 0),nil,font2,kHexColor(0x0C8918),(0),self.contentView);
    _La_des = JELa(JR(_La_index.x, _La_param.bottom, _La_API.width,1),nil,font2,nil,(0),self.contentView);
    return self;
}
...
-(UILabel *)La_rep{
    if (_La_rep == nil) {
        _La_rep = JELa(JR(jkLabelMargin/2, 0, 30, 30),nil,fontM(15),kColorRed,(1),self.contentView);
...
- (void)viewDidLoad{
    ...
    self.liteTv = [JELiteTV F:self.tvFrame style:(UITableViewStyleGrouped) cellC:JETableViewCell1.class cellH:ScrnAdaptMax(50) cell:^(__kindof UITableViewCell *cell, UITableView *tv, NSIndexPath *idx, JEDebugToolTimeListModel *mod) {
        ...
        cell.textLabel.text = [mod.date sameDayWithDate:date] ? mod.date.je_HHmmss : mod.date.je_YYYYMMddHHmmss;
...
- (void)viewDidLoad{
    ...
    [self je_rightBarBtn:@"更多" act:@selector(actionHandle)];//❖
...
- (void)actionHandle{
    ...
    [JEPutDownMenuView ShowIn:JEApp.window Point:CGPointMake(ScreenWidth, ScreenNavBarH) List:list Click:^(NSString *str, NSInteger index) {
        if ([str isEqualToString:cleanAll]){
            ...
            [[JEDebugTool__ Shared] closeOpen];
            [JEKit ShowAlert:@"发送LOG至开发者邮箱？" msg:nil style:(UIAlertControllerStyleAlert) block:^(NSString *actions, NSInteger index) {
                if (index == 0) {
                    [self sendLogToEmail];
                }
            } cancel:@"取消" actions:@[@"发送"] destructive:nil];
        }
        else if ([str isEqualToString:die]){
            [[JEDebugTool__ Shared] closeOpen];
            [JEKit ShowAlert:@"Sure ?" msg:@"清除所有本地缓存数据并关闭APP" style:(UIAlertControllerStyleAlert) block:^(NSString *actions, NSInteger index) {
                if (index == 1) {return ;}
            ...
            } cancel:@"NO" actions:@[@"Sure"] destructive:@[@"Sure"]];
        }
        
    } PositionUp:NO arrowX:0.85];
}
...
- (NSString*)deviceName{
    ...
    if ([_ isEqualToString:@"i386"] || [_ isEqualToString:@"x86_64"]) {
        return @"Simulator";
    }
    //iPhone
    if ([_ isEqualToString:@"iPhone1,1"])    return @"iPhone 1G";
    if ([_ isEqualToString:@"iPhone1,2"])    return @"iPhone 3G";
    if ([_ isEqualToString:@"iPhone2,1"])    return @"iPhone 3GS";
    if ([_ isEqualToString:@"iPhone3,1"])    return @"iPhone 4";
    if ([_ isEqualToString:@"iPhone3,2"])    return @"Verizon iPhone 4";
    if ([_ isEqualToString:@"iPhone4,1"])    return @"iPhone 4S";
    if ([_ isEqualToString:@"iPhone5,1"])    return @"iPhone 5";
    if ([_ isEqualToString:@"iPhone5,2"])    return @"iPhone 5";
    if ([_ isEqualToString:@"iPhone5,3"])    return @"iPhone 5C";
    if ([_ isEqualToString:@"iPhone5,4"])    return @"iPhone 5C";
    if ([_ isEqualToString:@"iPhone6,1"])    return @"iPhone 5S";
    if ([_ isEqualToString:@"iPhone6,2"])    return @"iPhone 5S";
    if ([_ isEqualToString:@"iPhone7,1"])    return @"iPhone 6 Plus";
    if ([_ isEqualToString:@"iPhone7,2"])    return @"iPhone 6";
    if ([_ isEqualToString:@"iPhone8,1"])    return @"iPhone 6s";
    if ([_ isEqualToString:@"iPhone8,2"])    return @"iPhone 6s Plus";
    if ([_ isEqualToString:@"iPhone9,1"] || [_ isEqualToString:@"iPhone9,3"])    return @"iPhone 7";
    if ([_ isEqualToString:@"iPhone9,2"] || [_ isEqualToString:@"iPhone9,4"])    return @"iPhone 7 Plus";
    if ([_ isEqualToString:@"iPhone10,1"] || [_ isEqualToString:@"iPhone10,4"])    return @"iPhone 8";
    if ([_ isEqualToString:@"iPhone10,2"] || [_ isEqualToString:@"iPhone10,5"])    return @"iPhone 8 Plus";
    if ([_ isEqualToString:@"iPhone10,3"] || [_ isEqualToString:@"iPhone10,6"])    return @"iPhone X";
    if ([_ isEqualToString:@"iPhone11,8"])   return @"iPhone XR";
    if ([_ isEqualToString:@"iPhone11,2"])   return @"iPhone XS";
    if ([_ isEqualToString:@"iPhone11,4"] || [_ isEqualToString:@"iPhone11,6"])    return @"iPhone XS Max";
...
- (void)loadDebugView{
    ...
    self.nav.je_navBarItemColor = kColorBlue;
    self.nav.je_titleColor = kColorText33;
