...
- (id)copyWithZone:(NSZone *)zone {
    JEPhotoGroupItem *item = [self.class new];
    return item;
}
@end
...
#pragma mark -   🔷🔷🔷🔷🔷🔷🔷🔷   JEPhotoGroupCell   🔷🔷🔷🔷🔷🔷🔷🔷
@interface JEPhotoGroupCell : UIScrollView <UIScrollViewDelegate>
...
@property (nonatomic, strong) JEPhotoGroupItem *item;
@property (nonatomic, readonly) BOOL itemDidLoad;
- (void)resizeSubviewSize;
@end

@implementation JEPhotoGroupCell
...
- (void)setItem:(JEPhotoGroupItem *)item {
...
@interface JEPhotoGroupView() <UIScrollViewDelegate, UIGestureRecognizerDelegate>
...
@implementation JEPhotoGroupView
...
//    _blurBackground.autoresizingMask = UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleHeight;
    ...
    _pager.height = 10;
    _pager.center = CGPointMake(self.width / 2, self.height - 18 - ScreenSafeArea);
    ...
    //    UIButton *btn = [UIButton Frame:CGRectMake(ScreenWidth - 70, 24, 50, 50) Title:@"删除" FontS:14 Color:[UIColor lightGrayColor] radius:25 Target:self action:@selector(jeDeleteImg) Bimg:[UIColor whiteColor]];
...
- (void)jeDeleteImg{
    if (_groupItems.count > _pager.currentPage) {
        [_groupItems removeObjectAtIndex:_pager.currentPage];
    }
    if (_groupItems.count == 0) {
        [self dismissAnimated:NO completion:nil];return;
    }
    if (_groupItems.count > _pager.currentPage - 1) {
        _fromView = _groupItems[_pager.currentPage - 1].thumbView;
    }else{
        _fromView = _groupItems.firstObject.thumbView;
    }
    
    [self dismissAnimated:NO completion:nil];
    //    [self removeFromSuperview];
    
    JEPhotoGroupView *v = [[JEPhotoGroupView alloc] initWithGroupItems:_groupItems];
    [v presentFromImageView:_fromView toContainer:_toContainerView animated:NO completion:nil];
}
...
- (void)presentFromImageView:(UIView *)fromView
    ...
    for (NSUInteger i = 0; i < self.groupItems.count; i++) {
        if (fromView == ((JEPhotoGroupItem *)self.groupItems[i]).thumbView) {
            page = (int)i;
            break;
        }
    }
...
//    _blurBackground.image = [_snapshorImageHideFromView imageByBlurLight];
    ...
    JEPhotoGroupCell *cell = [self cellForPage:self.currentPage];
    JEPhotoGroupItem *item = _groupItems[self.currentPage];
...
- (void)dismissAnimated:(BOOL)animated completion:(void (^)(void))completion {
    ...
    JEPhotoGroupCell *cell = [self cellForPage:currentPage];
    JEPhotoGroupItem *item;
    if (_groupItems.count) {
        item = _groupItems[currentPage];
    }
...
- (void)cancelAllImageLoad {
    [_cells enumerateObjectsUsingBlock:^(JEPhotoGroupCell *cell, NSUInteger idx, BOOL *stop) {
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    for (NSInteger i = page - 1; i <= page + 1; i++) { // preload left and right cell
        if (i >= 0 && i < self.groupItems.count) {
            JEPhotoGroupCell *cell = [self cellForPage:i];
            if (!cell) {
                JEPhotoGroupCell *cell = [self dequeueReusableCell];
                cell.page = i;
    ...
    self -> _pager.currentPage = intPage;
...
- (void)updateCellsForReuse {
    for (JEPhotoGroupCell *cell in _cells) {
        if (cell.superview) {
...
- (JEPhotoGroupCell *)dequeueReusableCell {
    JEPhotoGroupCell *cell = nil;
    for (cell in _cells) {
        if (!cell.superview) {
            return cell;
        }
...
- (JEPhotoGroupCell *)cellForPage:(NSInteger)page {
    for (JEPhotoGroupCell *cell in _cells) {
        if (cell.page == page) {
            return cell;
        }
...
- (void)doubleTap:(UITapGestureRecognizer *)g {
    ...
    JEPhotoGroupCell *tile = [self cellForPage:self.currentPage];
    if (tile) {
        if (tile.zoomScale > 1) {
...
- (void)longPress {
    ...
    if (!toVC) toVC = JEApp.window.rootViewController;
